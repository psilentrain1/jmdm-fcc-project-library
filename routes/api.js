...
module.exports = function (app) {
  app
    ...
    .post(async function (req, res) {
      ...
      if (!title || title === "") return res.status(400).send("missing required field title");
    ...
    .delete(async function (req, res) {
      ...
      try {
        ...
        if (deletedBooks.deletedCount === 0) return res.status(404).send("no books exist");
  ...
  app
    ...
    .post(async function (req, res) {
      ...
      if (!book) return res.status(404).send("no book exists");
      if (!comment || comment === "") return res.status(400).send("missing required field comment");
